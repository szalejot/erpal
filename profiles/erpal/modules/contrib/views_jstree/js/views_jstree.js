        ...
        "crrm" : {
            "move" : {
                ...
                "check_move" : function (m) { 
                  ...
                  var parent_id = parent_id.replace("node_","");     
            ...
            "success" : function(data, textStatus, jqXHR) {
              ...
              $.each(data, function(index, data_obj) {
                ...
                $.each(context_menu_items, function(item_id, item) {
                  ...
                  menus_by_id[key][item_id]["attributes"] = attributes; 
                });
              });
  ...
  function customMenu(node, parent_nid) {
    ...
    for (key in menu) {
      ...
      if (typeof title != 'undefined' && typeof url != 'undefined') {
        menu_items[item_key] = customMenuNode(title, url, attributes);
      }
    }
    return menu_items;
  }
  
  function customMenuNode(title, url, attributes) {
    var menu_object = {          
      label: title,
      action: function (clicked_node) {          
        var link = url;
        if (attributes && attributes.target == '_blank') {
